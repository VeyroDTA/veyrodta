    ...
    <section className="max-w-6xl mx-auto px-6 py-20">
      ...
      {pillarPost && (
        <Link
          ...
          className="group block mb-10 bg-gradient-to-r from-blue-600 via-cyan-600 to-teal-600 border border-cyan-400/40 rounded-3xl p-8 sm:p-10 shadow-[0_0_50px_rgba(6,182,212,0.25)] hover:shadow-[0_0_40px_rgba(6,182,212,0.22)] transition-all"
        >
          <div className="flex flex-col sm:flex-row items-start gap-8">
            <div className="flex-shrink-0 w-16 h-16 rounded-2xl bg-white/10 border border-white/30 flex items-center justify-center text-3xl">
              📖
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {articles.map((post) => (
          <Link
            key={post.slug}
            to={`/blog/${post.slug}`}
            className="group bg-[#0D1736] border border-cyan-500/20 rounded-2xl p-7 shadow-xl hover:border-cyan-400/50 transition-all flex flex-col justify-between"
          >
            <div>
